...
|---|---|---:|---:|---:|---|
| `vertex_detect_a100` | `yolo11x.pt` | 1280 | 8 | 180 | Main train accuracy-first |
| `vertex_detect_finetune_a100` | Stage 1 `best.pt` | 1280 | 8 | 60 | Fine-tune no-mosaic |

So với L4, A100 không chỉ giúp train nhanh hơn mà còn cho phép tăng `imgsz` từ `1024` lên `1280`. Với fire/smoke, điểm này quan trọng vì nhiều vùng lửa/khói nhỏ có thể bị mất chi tiết ở image size thấp. Batch size tăng từ `4` lên `8` giúp gradient ổn định hơn, nhưng vẫn giữ optimizer/lr tương tự để tránh thay đổi quá nhiều biến cùng lúc.
...
| Main batch | 4 | 8 | Batch lớn hơn, gradient ổn định hơn |
| Fine-tune image size | 1024 | 1280 | Đánh giá và refine ở độ phân giải cao hơn |
| Fine-tune batch | 4 | 8 | Fine-tune nhanh và ổn định hơn |
...
Trên L4, model chạy ở `imgsz=1024`, batch 4. Trên A100 40GB, cùng kiến trúc YOLO11x được tăng lên `imgsz=1280`, batch 8 để tận dụng VRAM và giữ chi tiết tốt hơn cho vùng lửa/khói nhỏ.
...
- Bổ sung profile A100 40GB để train ở `imgsz=1280`, batch lớn hơn và ưu tiên mAP.
...
Nếu A100 40GB bị OOM ở `batch=8`, giảm batch nhưng giữ `imgsz=1280` trước:
