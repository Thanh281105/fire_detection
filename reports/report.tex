...
Large local dataset files are omitted from the submission archive; they can be regenerated from the public repository using the provided preparation scripts.
...
For practical testing, the project includes a Streamlit interface in \code{app.py}. The interface loads the checkpoint, accepts uploaded images or bundled sample images, allows confidence, IoU, image-size, and device settings to be adjusted, and displays both the original image and the annotated prediction output. Figure~\ref{fig:streamlit-inference-ui} shows the interface on \code{Chay-Rung-3.jpg}. This UI is intended as a reproducible inference test tool, not as a production monitoring system.

\begin{figure}[!h]
\centering
\safeincludegraphics[width=0.86\textwidth]{figures/streamlit_inference_ui.png}
...
Figures~\ref{fig:class-distribution}--\ref{fig:wh-scatter} highlight three EDA points: \code{g01} dominates the metric, \code{g03}/\code{g05} add background cases for false-alarm control, and \code{g04} contributes dense small-object examples for early-fire evaluation.

% ===================== CHAPTER 4 =====================
\chapter{Data Preprocessing}
\section{Pipeline Overview}
The preprocessing pipeline is implemented by the project preparation and audit scripts. Figure~\ref{fig:pipeline} summarizes the workflow from raw image groups to audited YOLO labels.
...
Although the final metric is not high, the project is valuable as a rigorous experimental pipeline. It documents dataset construction, detector training, rented GPU experiments, checkpoint packaging, a usable inference UI, and error analysis. The remaining weaknesses are explicit: missing confidence/NMS sweeps, quantitative annotation-quality measurement, background false-positive measurement, and a small-object sliced-inference protocol gap. This makes the report stronger than a simple high-score demo because both the engineering work and research risks are clear.
